...
async def test_get_article_comments(
    mock_article_crud, mock_comment_crud, app, test_article, test_comment_with_user
):
    # GIVEN article exists with comments
    mock_article_crud.get_article_by_id = AsyncMock(return_value=test_article)
    mock_comment_crud.get_comments_by_article = AsyncMock(
        return_value=[test_comment_with_user]
    )
...
@patch("app.routes.comments.comment_crud")
@patch("app.routes.comments.article_crud")
async def test_get_article_comments_with_pagination(
    mock_article_crud, mock_comment_crud, app, test_article
):
    ...
    async with AsyncClient(
        ...
        response = await client.get(
            f"/articles/{test_article['id']}/comments", params={"skip": 10, "limit": 5}
        )
...
async def test_create_comment_unauthorized(app, test_article):
    ...
    async with AsyncClient(
        ...
        # WHEN create comment is called
        response = await client.post(
            f"/articles/{test_article['id']}/comments", json={"content": "New comment"}
        )
...
async def test_update_comment(
    mock_verify, mock_get_user, mock_comment_crud, app, test_user, test_comment
):
...
async def test_update_comment_not_found(
    mock_verify, mock_get_user, mock_comment_crud, app, test_user
):
...
async def test_delete_comment(
    ...
    mock_article_crud.increment_comment_count.assert_awaited_with(
        test_comment["article_id"], -1
    )
...
async def test_delete_comment_not_found(
    ...
    async with AsyncClient(
        ...
        # WHEN delete comment is called
        response = await client.delete(
            "/comments/nonexistent-id", headers={"Authorization": "Bearer valid-token"}
        )
...
@patch("app.dependencies.get_user_by_id")
@patch("app.dependencies.verify_token")
async def test_get_my_comments(
    mock_verify,
    mock_get_user,
    mock_comment_crud,
    app,
    ...
    mock_verify.return_value = {"sub": test_user["id"]}
    mock_get_user.return_value = test_user
    mock_comment_crud.get_comments_by_user = AsyncMock(
        return_value=[test_comment_with_user]
    )
    ...
    async with AsyncClient(
        ...
        response = await client.get(
            "/users/me/comments", headers={"Authorization": "Bearer valid-token"}
        )
...
async def test_get_user_comments(
    mock_comment_crud, app, test_user, test_comment_with_user
):

...
from app.schemas.topic import Topic, TopicBulkFollow, TopicCreate
...
async def bulk_follow_topics(
    input_data: TopicBulkFollow, current_user: dict = Depends(get_current_user)
):
    topic_ids = input_data.topic_ids
    if len(topic_ids) < 3:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Minimum 3 topics required"
        )

...
TOPIC_MAPPING = {
    ...
    "science": 'text:science OR category:"Science and Technology"',
...
async def fetch_news(
    ...
    try:
        ...
        if isinstance(e, httpx.HTTPStatusError):
             raise HTTPException(status_code=e.response.status_code, detail=f"Webz.io API error: {e.response.text}")
        raise e
...
async def fetch_news_feed(
    ...
    """
    if not topics:
        return await fetch_news(query="news", timestamp=timestamp, size=size, country=country)
    ...
    print(f"DEBUG: Generated news feed query ({len(combined_query)} chars): {combined_query}")
        
    return await fetch_news(query=combined_query, timestamp=timestamp, size=size, country=country)
...
async def fetch_news_by_topic(
    ...
    return await fetch_news(query=query, timestamp=timestamp, size=size, country=country)
